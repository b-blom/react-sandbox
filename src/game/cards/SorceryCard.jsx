	...
	return (
		<div className='playing-card support-card'>
			...
			>
				...
				<div className='card-image'>{props.card.sorceryImage}</div>
			</button>
			{showMenu && (
				<div className='ability-button sorcery-selected'>
					{props.activeCard(props.card, props.id)}
					<p className='small-font'>{props.card.sorceryDescription}</p>
					<p className='small-font'>cost: {props.card.sorceryCost}</p>
				</div>
			)}
		</div>
	);
