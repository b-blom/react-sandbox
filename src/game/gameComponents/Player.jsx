...
export default function Player(props) {
  const { msg, setMsg, player1Name } = useContext(SelectedSorceryCardContext);
    ...
    <div className='player'>
      ...
      <div className='row'>
        ...
        <div className='health-adjust-wrapper '>
          <button
            onClick={() => {
              props.setPlayerHp(props.playerHp - 1);
            }}>
            -
          </button>
          <button
      ...
      <div className='mana-wrapper'>
        {props.manaDeck &&
          props.manaDeck.map((card, index) => {
            return <ManaCard id={index} card={card} />;
          })}
      </div>
      <div className='creature-wrapper'>
        {props.creatureDeck &&
          props.creatureDeck.map((card, index) => {
            return (
              <CreatureCard
      ...
      <div className='support-cards-wrapper column'>
        <div className='support-card-type-wrapper'>
          {props.instantsDeck &&
            props.instantsDeck.map((card, index) => {
              return (
                <InstantCard
                  id={index}
                  card={card}
        ...
        <div className='support-card-type-wrapper'>
          {props.sorceryDeck &&
            props.sorceryDeck.map((card, index) => {
